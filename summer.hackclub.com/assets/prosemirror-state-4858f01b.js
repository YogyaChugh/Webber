...
class Selection {
    ...
    replace(t, n = e.empty) {
        ...
        for (let l = 0; l < s.length; l++) {
            let {
                $from: c,
                $to: a
            } = s[l], h = t.mapping.slice(o);
            t.replaceRange(h.map(c.pos), h.map(a.pos), l ? e.empty : n);
            0 == l && selectionToInsertionEnd(t, o, (r ? r.isInline : i && i.isTextblock) ? -1 : 1)
        }
    }
    replaceWith(e, t) {
        ...
        for (let i = 0; i < r.length; i++) {
            let {
                $from: o,
                $to: s
            } = r[i], l = e.mapping.slice(n);
            let c = l.map(o.pos),
                a = l.map(s.pos);
            if (i) e.deleteRange(c, a);
            else {
                e.replaceRangeWith(c, a, t);
                selectionToInsertionEnd(e, n, t.isInline ? -1 : 1)
            }
        }
    }
    static findFrom(e, t, n = false) {
        let r = e.parent.inlineContent ? new TextSelection(e) : findSelectionIn(e.node(0), e.parent, e.pos, e.index(), t, n);
        if (r) return r;
        for (let r = e.depth - 1; r >= 0; r--) {
            let i = t < 0 ? findSelectionIn(e.node(0), e.node(r), e.before(r + 1), e.index(r), t, n) : findSelectionIn(e.node(0), e.node(r), e.after(r + 1), e.index(r) + 1, t, n);
            if (i) return i
        }
        return null
    }
    static near(e, t = 1) {
        return this.findFrom(e, t) || this.findFrom(e, -t) || new AllSelection(e.node(0))
    }
    static atStart(e) {
    ...
    static jsonID(e, t) {
        if (e in l) throw new RangeError("Duplicate use of selection JSON ID " + e);
        l[e] = t;
...
class TextSelection extends Selection {
    ...
    map(e, t) {
        ...
        if (!n.parent.inlineContent) return Selection.near(n);
        let r = e.resolve(t.map(this.anchor));
        return new TextSelection(r.parent.inlineContent ? r : n, n)
    }
    replace(t, n = e.empty) {
        super.replace(t, n);
        if (n == e.empty) {
    ...
    eq(e) {
        return e instanceof TextSelection && e.anchor == this.anchor && e.head == this.head
    }
    getBookmark() {
    ...
    static between(e, t, n) {
        ...
        if (!t.parent.inlineContent) {
            let e = Selection.findFrom(t, n, true) || Selection.findFrom(t, -n, true);
            if (!e) return Selection.near(t, n);
            t = e.$head
        }
        if (!e.parent.inlineContent)
            if (0 == r) e = t;
            else {
                e = (Selection.findFrom(e, -n, true) || Selection.findFrom(e, n, true)).$anchor;
                e.pos < t.pos != r < 0 && (e = t)
            } return new TextSelection(e, t)
    }
}
...
class NodeSelection extends Selection {
    ...
    map(e, t) {
        ...
        let i = e.resolve(r);
        return n ? Selection.near(i) : new NodeSelection(i)
    }
    content() {
    ...
    static isSelectable(e) {
        return !e.isText && false !== e.type.spec.selectable
    }
}
...
class NodeBookmark {
    ...
    map(e) {
        ...
        return t ? new TextBookmark(n, n) : new NodeBookmark(n)
    }
    resolve(e) {
        let t = e.resolve(this.anchor),
            n = t.nodeAfter;
        return n && NodeSelection.isSelectable(n) ? new NodeSelection(t) : Selection.near(t)
    }
}
...
function findSelectionIn(e, t, n, r, i, o = false) {
    if (t.inlineContent) return TextSelection.create(e, n);
    for (let s = r - (i > 0 ? 0 : 1); i > 0 ? s < t.childCount : s >= 0; s += i) {
        let r = t.child(s);
        if (r.isAtom) {
            if (!o && NodeSelection.isSelectable(r)) return NodeSelection.create(e, n - (i < 0 ? r.nodeSize : 0))
        } else {
            let t = findSelectionIn(e, r, n + i, i < 0 ? r.childCount : 0, i, o);
            if (t) return t
        }
        n += r.nodeSize * i
    }
...
function selectionToInsertionEnd(e, t, n) {
    let r = e.steps.length - 1;
    if (r < t) return;
    let s = e.steps[r];
    if (!(s instanceof i || s instanceof o)) return;
    let l, c = e.mapping.maps[r];
    c.forEach(((e, t, n, r) => {
        null == l && (l = r)
    }));
    e.setSelection(Selection.near(e.doc.resolve(l), n))
}
const h = 1,
    u = 2,
    d = 4;
class Transaction extends s {
    constructor(e) {
        super(e.doc);
    ...
    ensureMarks(e) {
        n.sameSet(this.storedMarks || this.selection.$from.marks(), e) || this.setStoredMarks(e);
        return this
    }
    addStoredMark(e) {
    ...
    replaceSelectionWith(e, t = true) {
        ...
        t && (e = e.mark(this.storedMarks || (r.empty ? r.$from.marks() : r.$from.marksAcross(r.$to) || n.none)));
    ...
    insertText(e, t, n) {
        ...
        if (null == t) return e ? this.replaceSelectionWith(r.text(e), true) : this.deleteSelection();
        {
            null == n && (n = t);
            n = null == n ? t : n;
            if (!e) return this.deleteRange(t, n);
    ...
    setMeta(e, t) {
        this.meta["string" == typeof e ? e : e.key] = t;
        return this
    }
    getMeta(e) {
...
class Configuration {
    constructor(e, t) {
        ...
        t && t.forEach((e => {
            ...
            e.spec.state && this.fields.push(new FieldDesc(e.key, e.spec.state, e))
        }))
    }
}
class EditorState {
    ...
    filterTransaction(e, t = -1) {
        for (let n = 0; n < this.config.plugins.length; n++)
            if (n != t) {
                ...
                if (t.spec.filterTransaction && !t.spec.filterTransaction.call(t, e, this)) return false
            } return true
    }
    applyTransaction(e) {
        ...
        for (;;) {
            ...
            for (let o = 0; o < this.config.plugins.length; o++) {
                ...
                if (s.spec.appendTransaction) {
                    let l = r ? r[o].n : 0,
                        c = r ? r[o].state : this;
                    let a = l < t.length && s.spec.appendTransaction.call(s, l ? t.slice(l) : t, c, n);
                    if (a && n.filterTransaction(a, o)) {
                        a.setMeta("appendedTransaction", e);
                        if (!r) {
                            r = [];
                            for (let e = 0; e < this.config.plugins.length; e++) r.push(e < o ? {
                                state: n,
                                n: t.length
                            } : {
                                state: this,
    ...
    applyInner(e) {
        ...
        let t = new EditorState(this.config),
    ...
    static create(e) {
        let t = new Configuration(e.doc ? e.doc.type.schema : e.schema, e.plugins);
        let n = new EditorState(t);
        for (let r = 0; r < t.fields.length; r++) n[t.fields[r].name] = t.fields[r].init(e, n);
        return n
    }
    reconfigure(e) {
        let t = new Configuration(this.schema, e.plugins);
        let n = t.fields,
            r = new EditorState(t);
    ...
    static fromJSON(e, t, n) {
        ...
        let i = new Configuration(e.schema, e.plugins);
        let o = new EditorState(i);
        i.fields.forEach((i => {
            if ("doc" == i.name) o.doc = r.fromJSON(e.schema, t.doc);
            else if ("selection" == i.name) o.selection = Selection.fromJSON(o.doc, t.selection);
                ...
                if (n)
                    for (let r in n) {
                        let s = n[r],
                            l = s.spec.state;
                        if (s.key == i.name && l && l.fromJSON && Object.prototype.hasOwnProperty.call(t, r)) {
                            o[i.name] = l.fromJSON.call(s, e, t[r], o);
                            return
                        }
                    }
                o[i.name] = i.init(e, o)
...
function bindProps(e, t, n) {
    for (let r in e) {
        let i = e[r];
        i instanceof Function ? i = i.bind(t) : "handleDOMEvents" == r && (i = bindProps(i, t, {}));
        n[r] = i
    }
    return n
}
