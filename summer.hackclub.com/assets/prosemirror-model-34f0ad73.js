    ...
    for (let s = t.childCount, o = e.childCount;;) {
        ...
        if (l != h) {
            ...
            if (l.isText && l.text != h.text) {
                ...
                while (t < e && l.text[l.text.length - t - 1] == h.text[h.text.length - t - 1]) {
                    t++;
                    r--;
                    i--
                }
            ...
            if (l.content.size || h.content.size) {
                let t = n(l.content, h.content, r - 1, i - 1);
                if (t) return t
            }
            r -= a;
            i -= a
...
class Fragment {
    ...
    nodesBetween(t, e, n, r = 0, i) {
        for (let s = 0, o = 0; o < e; s++) {
            let l = this.content[s],
                h = o + l.nodeSize;
            if (h > t && n(l, r + o, i || null, s) !== false && l.content.size) {
                let i = o + 1;
    ...
    textBetween(t, e, n, r) {
        let i = "",
            s = true;
        this.nodesBetween(t, e, ((o, l) => {
            let h = o.isText ? o.text.slice(Math.max(t, l) - l, e - l) : o.isLeaf ? r ? typeof r === "function" ? r(o) : r : o.type.spec.leafText ? o.type.spec.leafText(o) : "" : "";
            o.isBlock && (o.isLeaf && h || o.isTextblock) && n && (s ? s = false : i += n);
            i += h
        }), 0);
        return i
    }
    append(t) {
        ...
        let e = this.lastChild,
            ...
            i = 0;
        if (e.isText && e.sameMarkup(n)) {
            r[r.length - 1] = e.withText(e.text + n.text);
            i = 1
        }
        for (; i < t.content.length; i++) r.push(t.content[i]);
        return new Fragment(r, this.size + t.size)
    }
    cut(t, e = this.size) {
        ...
        if (e > t)
            for (let i = 0, s = 0; s < e; i++) {
                let o = this.content[i],
                    l = s + o.nodeSize;
                if (l > t) {
                    (s < t || l > e) && (o = o.isText ? o.cut(Math.max(0, t - s), Math.min(o.text.length, e - s)) : o.cut(Math.max(0, t - s - 1), Math.min(o.content.size, e - s - 1)));
                    n.push(o);
                    r += o.nodeSize
                }
                s = l
    ...
    cutByIndex(t, e) {
        return t == e ? Fragment.empty : t == 0 && e == this.content.length ? this : new Fragment(this.content.slice(t, e))
    }
    replaceChild(t, e) {
        let n = this.content[t];
        if (n == e) return this;
        let r = this.content.slice();
        let i = this.size + e.nodeSize - n.nodeSize;
        r[t] = e;
        return new Fragment(r, i)
    }
    addToStart(t) {
    ...
    eq(t) {
        if (this.content.length != t.content.length) return false;
        for (let e = 0; e < this.content.length; e++)
            if (!this.content[e].eq(t.content[e])) return false;
        return true
    }
    get firstChild() {
    ...
    findIndex(t, e = -1) {
        ...
        for (let n = 0, r = 0;; n++) {
            let s = this.child(n),
                o = r + s.nodeSize;
            if (o >= t) return o == t || e > 0 ? i(n + 1, o) : i(n, r);
            r = o
        }
    }
    toString() {
    ...
    static fromJSON(t, e) {
        ...
        return new Fragment(e.map(t.nodeFromJSON))
    }
    static fromArray(t) {
        ...
        for (let r = 0; r < t.length; r++) {
            let i = t[r];
            n += i.nodeSize;
            if (r && i.isText && t[r - 1].sameMarkup(i)) {
                e || (e = t.slice(0, r));
                e[e.length - 1] = i.withText(e[e.length - 1].text + i.text)
            } else e && e.push(i)
        }
        return new Fragment(e || t, n)
    }
    static from(t) {
...
function s(t, e) {
    if (t === e) return true;
    if (!(t && typeof t == "object") || !(e && typeof e == "object")) return false;
    let n = Array.isArray(t);
    if (Array.isArray(e) != n) return false;
    if (n) {
        if (t.length != e.length) return false;
        for (let n = 0; n < t.length; n++)
            if (!s(t[n], e[n])) return false
    } else {
        for (let n in t)
            if (!(n in e) || !s(t[n], e[n])) return false;
        for (let n in e)
...
class Mark {
    ...
    addToSet(t) {
        ...
        for (let r = 0; r < t.length; r++) {
            ...
            if (this.type.excludes(i.type)) e || (e = t.slice(0, r));
                ...
                if (!n && i.type.rank > this.type.rank) {
                    e || (e = t.slice(0, r));
                    e.push(this);
                    n = true
                }
    ...
    static setFrom(t) {
        ...
        let e = t.slice();
        e.sort(((t, e) => t.type.rank - e.type.rank));
        return e
    }
}
Mark.none = [];
...
class Slice {
    ...
    insertAt(t, e) {
        let n = l(this.content, t + this.openStart, e);
        return n && new Slice(n, this.openStart, this.openEnd)
    }
    removeBetween(t, e) {
        return new Slice(o(this.content, t + this.openStart, e + this.openStart), this.openStart, this.openEnd)
    }
    eq(t) {
    ...
    static fromJSON(t, e) {
        if (!e) return Slice.empty;
        let n = e.openStart || 0,
            r = e.openEnd || 0;
        if (typeof n != "number" || typeof r != "number") throw new RangeError("Invalid input for Slice.fromJSON");
        return new Slice(Fragment.fromJSON(t, e.content), n, r)
    }
    static maxOpen(t, e = true) {
        let n = 0,
            r = 0;
...
function o(t, e, n) {
    let {
        index: r,
        offset: i
    } = t.findIndex(e), s = t.maybeChild(r);
    let {
        index: l,
        offset: h
    } = t.findIndex(n);
    if (i == e || s.isText) {
        if (h != n && !t.child(l).isText) throw new RangeError("Removing non-flat range");
        return t.cut(0, e).append(t.cut(n))
    }
    if (r != l) throw new RangeError("Removing non-flat range");
    return t.replaceChild(r, s.copy(o(s.content, e - i - 1, n - i - 1)))
}
...
function l(t, e, n, r) {
    let {
        index: i,
        offset: s
    } = t.findIndex(e), o = t.maybeChild(i);
    if (s == e || o.isText) return r && !r.canReplace(i, i, n) ? null : t.cut(0, e).append(n).append(t.cut(e));
    let h = l(o.content, e - s - 1, n);
    return h && t.replaceChild(i, o.copy(h))
}

function h(t, e, n) {
    if (n.openStart > t.depth) throw new ReplaceError("Inserted content deeper than insertion position");
    if (t.depth - n.openStart != e.depth - n.openEnd) throw new ReplaceError("Inconsistent open depths");
    return a(t, e, n, 0)
}
...
function a(t, e, n, r) {
    ...
    if (n.content.size) {
        if (n.openStart || n.openEnd || t.depth != r || e.depth != r) {
            ...
            return d(s, m(t, i, o, e, r))
        } {
            let r = t.parent,
                i = r.content;
            return d(r, i.cut(0, t.parentOffset).append(n.content).append(i.cut(e.parentOffset)))
        }
    }
    return d(s, g(t, e, r))
}
...
function c(t, e) {
    if (!e.type.compatibleContent(t.type)) throw new ReplaceError("Cannot join " + e.type.name + " onto " + t.type.name)
}
...
function p(t, e, n, r) {
    ...
    if (t) {
        s = t.index(n);
        if (t.depth > n) s++;
        else if (t.textOffset) {
            f(t.nodeAfter, r);
            s++
        }
    }
    for (let t = s; t < o; t++) f(i.child(t), r);
    e && e.depth == n && e.textOffset && f(e.nodeBefore, r)
}

function d(t, e) {
    t.type.checkContent(e);
    return t.copy(e)
}

function m(t, e, n, r, i) {
    let s = t.depth > i && u(t, e, i + 1);
    let o = r.depth > i && u(n, r, i + 1);
    let l = [];
    p(null, t, i, l);
    if (s && o && e.index(i) == n.index(i)) {
        c(s, o);
        f(d(s, m(t, e, n, r, i + 1)), l)
    } else {
        s && f(d(s, g(t, e, i + 1)), l);
        p(e, n, i, l);
        o && f(d(o, g(n, r, i + 1)), l)
...
function x(t, e) {
    let n = e.depth - t.openStart,
        r = e.node(n);
    let i = r.copy(t.content);
    for (let t = n - 1; t >= 0; t--) i = e.node(t).copy(Fragment.from(i));
    return {
...
class ResolvedPos {
    ...
    indexAfter(t) {
        ...
        return this.index(t) + (t != this.depth || this.textOffset ? 1 : 0)
    }
    start(t) {
        t = this.resolveDepth(t);
        return t == 0 ? 0 : this.path[t * 3 - 1] + 1
    }
    end(t) {
        t = this.resolveDepth(t);
        return this.start(t) + this.node(t).content.size
    }
    before(t) {
        t = this.resolveDepth(t);
        if (!t) throw new RangeError("There is no position before the top-level node");
        return t == this.depth + 1 ? this.pos : this.path[t * 3 - 1]
    }
    after(t) {
        t = this.resolveDepth(t);
        if (!t) throw new RangeError("There is no position after the top-level node");
    ...
    marks() {
        ...
        let i = n.marks;
        for (var s = 0; s < i.length; s++) i[s].type.spec.inclusive !== false || r && i[s].isInSet(r.marks) || (i = i[s--].removeFromSet(i));
        return i
    }
    marksAcross(t) {
        ...
        for (var i = 0; i < n.length; i++) n[i].type.spec.inclusive !== false || r && n[i].isInSet(r.marks) || (n = n[i--].removeFromSet(n));
        return n
    }
    sharedDepth(t) {
    ...
    blockRange(t = this, e) {
        ...
        for (let n = this.depth - (this.parent.inlineContent || this.pos == t.pos ? 1 : 0); n >= 0; n--)
            if (t.pos <= this.end(n) && (!e || e(this.node(n)))) return new NodeRange(this, t, n);
        return null
    }
    sameParent(t) {
    ...
    static resolveCached(t, e) {
        ...
        if (n)
            for (let t = 0; t < n.elts.length; t++) {
                let r = n.elts[t];
                if (r.pos == e) return r
            } else k.set(t, n = new ResolveCache);
        let r = n.elts[n.i] = ResolvedPos.resolve(t, e);
        n.i = (n.i + 1) % y;
        return r
    }
}
...
class Node {
    ...
    hasMarkup(t, e, n) {
        return this.type == t && s(this.attrs, e || t.defaultAttrs || w) && Mark.sameSet(this.marks, n || Mark.none)
    }
    copy(t = null) {
        return t == this.content ? this : new Node(this.type, this.attrs, t, this.marks)
    }
    mark(t) {
        return t == this.marks ? this : new Node(this.type, this.attrs, this.content, t)
    }
    cut(t, e = this.content.size) {
        return t == 0 && e == this.content.size ? this : this.copy(this.content.cut(t, e))
    }
    slice(t, e = this.content.size, n = false) {
        if (t == e) return Slice.empty;
        let r = this.resolve(t),
            i = this.resolve(e);
        let s = n ? 0 : r.sharedDepth(e);
        let o = r.start(s),
            l = r.node(s);
        let h = l.content.cut(r.pos - o, i.pos - o);
        return new Slice(h, r.depth - s, i.depth - s)
    }
    replace(t, e, n) {
    ...
    rangeHasMark(t, e, n) {
        let r = false;
        e > t && this.nodesBetween(t, e, (t => {
            n.isInSet(t.marks) && (r = true);
            return !r
        }));
        return r
    }
    ...
    canReplace(t, e, n = Fragment.empty, r = 0, i = n.childCount) {
        let s = this.contentMatchAt(t).matchFragment(n, r, i);
        let o = s && s.matchFragment(this.content, e);
        if (!o || !o.validEnd) return false;
        for (let t = r; t < i; t++)
            if (!this.type.allowsMarks(n.child(t).marks)) return false;
        return true
    }
    canReplaceWith(t, e, n, r) {
        if (r && !this.type.allowsMarks(r)) return false;
        let i = this.contentMatchAt(t).matchType(n);
        let s = i && i.matchFragment(this.content, e);
        return !!s && s.validEnd
    }
    canAppend(t) {
    ...
    check() {
        ...
        if (!Mark.sameSet(t, this.marks)) throw new RangeError(`Invalid collection of marks for node ${this.type.name}: ${this.marks.map((t=>t.type.name))}`);
    ...
    static fromJSON(t, e) {
        ...
        if (e.type == "text") {
            if (typeof e.text != "string") throw new RangeError("Invalid text node in JSON");
            return t.text(e.text, n)
        }
        let r = Fragment.fromJSON(t, e.content);
        let i = t.nodeType(e.type).create(e.attrs, r, n);
        i.type.checkAttrs(i.attrs);
        return i
    }
}
...
class TextNode extends Node {
    constructor(t, e, n, r) {
        super(t, e, null, r);
        if (!n) throw new RangeError("Empty text nodes are not allowed");
        this.text = n
    }
    toString() {
...
class ContentMatch {
    ...
    static parse(t, e) {
        let n = new TokenStream(t, e);
        if (n.next == null) return ContentMatch.empty;
        let r = M(n);
    ...
    compatible(t) {
        for (let e = 0; e < this.next.length; e++)
            for (let n = 0; n < t.next.length; n++)
                if (this.next[e].type == t.next[n].type) return true;
        return false
    }
    fillBefore(t, e = false, n = 0) {
        let r = [this];

        function i(s, o) {
            let l = s.matchFragment(t, n);
            if (l && (!e || l.validEnd)) return Fragment.from(o.map((t => t.createAndFill())));
            for (let t = 0; t < s.next.length; t++) {
                let {
                    type: e,
                    next: n
                } = s.next[t];
                if (!(e.isText || e.hasRequiredAttrs()) && r.indexOf(n) == -1) {
                    r.push(n);
    ...
    computeWrapping(t) {
        ...
        while (n.length) {
            ...
            for (let t = 0; t < i.next.length; t++) {
                ...
                if (!s.isLeaf && !s.hasRequiredAttrs() && !(s.name in e) && (!r.type || o.validEnd)) {
                    n.push({
    ...
    toString() {
        ...
        function e(n) {
            t.push(n);
            for (let r = 0; r < n.next.length; r++) t.indexOf(n.next[r].next) == -1 && e(n.next[r].next)
        }
        e(this);
        return t.map(((e, n) => {
            let r = n + (e.validEnd ? "*" : " ") + " ";
...
class TokenStream {
    ...
    eat(t) {
        return this.next == t && (this.pos++ || true)
    }
    err(t) {
...
function T(t, e) {
    ...
    i.length == 0 && t.err("No node type or group '" + e + "' found");
    return i
}
...
function v(t) {
    ...
    if (!/\W/.test(t.next)) {
        let e = T(t, t.next).map((e => {
            t.inline == null ? t.inline = e.isInline : t.inline != e.isInline && t.err("Mixing inline and block content");
            return {
                type: "name",
                value: e
            }
...
function N(t) {
    ...
    function s(t, e) {
        if (t.type == "choice") return t.exprs.reduce(((t, n) => t.concat(s(n, e))), []);
        if (t.type != "seq") {
            if (t.type == "star") {
                let o = n();
                r(e, o);
            ...
            if (t.type == "opt") return [r(e)].concat(s(t.expr, e));
            if (t.type == "range") {
                let o = e;
...
function F(t) {
    ...
    function n(r) {
        ...
        let s = e[r.join(",")] = new ContentMatch(r.indexOf(t.length - 1) > -1);
        for (let t = 0; t < i.length; t++) {
            let r = i[t][1].sort(A);
            s.next.push({
                type: i[t][0],
...
function z(t, e) {
    for (let n = 0, r = [t]; n < r.length; n++) {
        ...
        for (let e = 0; e < t.next.length; e++) {
            ...
            s.push(n.name);
            i && !(n.isText || n.hasRequiredAttrs()) && (i = false);
...
function D(t, e, n, r) {
    for (let r in e)
        if (!(r in t)) throw new RangeError(`Unsupported attribute ${r} for ${n} of type ${r}`);
    for (let n in t) {
        let r = t[n];
...
function B(t, e) {
    let n = Object.create(null);
    if (e)
        for (let r in e) n[r] = new Attribute(t, r, e[r]);
    return n
}
class NodeType {
    constructor(t, e, n) {
    ...
    hasRequiredAttrs() {
        for (let t in this.attrs)
            if (this.attrs[t].isRequired) return true;
        return false
    }
    compatibleContent(t) {
    ...
    computeAttrs(t) {
        return !t && this.defaultAttrs ? this.defaultAttrs : P(this.attrs, t)
    }
    create(t = null, e, n) {
        if (this.isText) throw new Error("NodeType.create can't construct text nodes");
        return new Node(this, this.computeAttrs(t), Fragment.from(e), Mark.setFrom(n))
    }
    createChecked(t = null, e, n) {
        e = Fragment.from(e);
        this.checkContent(e);
        return new Node(this, this.computeAttrs(t), e, Mark.setFrom(n))
    }
    createAndFill(t = null, e, n) {
        ...
        let i = r && r.fillBefore(Fragment.empty, true);
        return i ? new Node(this, t, e.append(i), Mark.setFrom(n)) : null
    }
    validContent(t) {
        let e = this.contentMatch.matchFragment(t);
        if (!e || !e.validEnd) return false;
        for (let e = 0; e < t.childCount; e++)
            if (!this.allowsMarks(t.child(e).marks)) return false;
        return true
    }
    checkContent(t) {
    ...
    allowsMarks(t) {
        ...
        for (let e = 0; e < t.length; e++)
            if (!this.allowsMarkType(t[e].type)) return false;
        return true
    }
    allowedMarks(t) {
        ...
        return e ? e.length ? e : Mark.none : t
    }
    static compile(t, e) {
        let n = Object.create(null);
        t.forEach(((t, r) => n[t] = new NodeType(t, e, r)));
...
class Attribute {
    constructor(t, e, n) {
        ...
        this.validate = typeof n.validate == "string" ? J(t, e, n.validate) : n.validate
    }
    get isRequired() {
...
class MarkType {
    constructor(t, e, n, r) {
        ...
        this.schema = n;
        this.spec = r;
        this.attrs = B(t, r.attrs);
        this.excluded = null;
        let i = I(this.attrs);
        this.instance = i ? new Mark(this, i) : null
    }
    create(t = null) {
        return !t && this.instance ? this.instance : new Mark(this, P(this.attrs, t))
    }
    static compile(t, e) {
        let n = Object.create(null),
            r = 0;
        t.forEach(((t, i) => n[t] = new MarkType(t, r++, e, i)));
        return n
    }
    removeFromSet(t) {
...
class Schema {
    constructor(e) {
        ...
        n.nodes = t.from(e.nodes), n.marks = t.from(e.marks || {}), this.nodes = NodeType.compile(this.spec.nodes, this);
        ...
        for (let t in this.nodes) {
            ...
            let e = this.nodes[t],
                n = e.spec.content || "",
                i = e.spec.marks;
            e.contentMatch = r[n] || (r[n] = ContentMatch.parse(n, this.nodes));
            e.inlineContent = e.contentMatch.inlineContent;
            if (e.spec.linebreakReplacement) {
            ...
            e.markSet = i == "_" ? null : i ? q(this, i.split(" ")) : i != "" && e.inlineContent ? null : []
        }
        for (let t in this.marks) {
            let e = this.marks[t],
                n = e.spec.excludes;
            e.excluded = n == null ? [e] : n == "" ? [] : q(this, n.split(" "))
        }
    ...
    node(t, e = null, n, r) {
        if (typeof t == "string") t = this.nodeType(t);
            ...
            if (!(t instanceof NodeType)) throw new RangeError("Invalid node type: " + t);
    ...
    text(t, e) {
        ...
        return new TextNode(n, n.defaultAttrs, t, Mark.setFrom(e))
    }
    mark(t, e) {
        typeof t == "string" && (t = this.marks[t]);
        return t.create(e)
    }
    nodeFromJSON(t) {
        return Node.fromJSON(this, t)
    }
    markFromJSON(t) {
...
class DOMParser {
    constructor(t, e) {
        ...
        this.normalizeLists = !this.tags.some((e => {
            if (!/^(ul|ol)\b/.test(e.tag) || !e.node) return false;
            let n = t.nodes[e.node];
            return n.contentMatch.matchType(n)
        }))
    }
    parse(t, e = {}) {
        let n = new ParseContext(this, e, false);
        n.addAll(t, Mark.none, e.from, e.to);
        return n.finish()
    }
    parseSlice(t, e = {}) {
        let n = new ParseContext(this, e, true);
        n.addAll(t, Mark.none, e.from, e.to);
        return Slice.maxOpen(n.finish())
    }
    matchTag(t, e, n) {
        for (let r = n ? this.tags.indexOf(n) + 1 : 0; r < this.tags.length; r++) {
            let n = this.tags[r];
            if (Q(t, n.tag) && (n.namespace === void 0 || t.namespaceURI == n.namespace) && (!n.context || e.matchesContext(n.context))) {
                if (n.getAttrs) {
                    let e = n.getAttrs(t);
                    if (e === false) continue;
                    n.attrs = e || void 0
    ...
    matchStyle(t, e, n, r) {
        for (let i = r ? this.styles.indexOf(r) + 1 : 0; i < this.styles.length; i++) {
            let r = this.styles[i],
                s = r.style;
            if (!(s.indexOf(t) != 0 || r.context && !n.matchesContext(r.context) || s.length > t.length && (s.charCodeAt(t.length) != 61 || s.slice(t.length + 1) != e))) {
                if (r.getAttrs) {
                    let t = r.getAttrs(e);
                    if (t === false) continue;
                    r.attrs = t || void 0
    ...
    static schemaRules(t) {
        ...
        function n(t) {
            let n = t.priority == null ? 50 : t.priority,
                r = 0;
            for (; r < e.length; r++) {
                let t = e[r],
                    i = t.priority == null ? 50 : t.priority;
                if (i < n) break
            }
            e.splice(r, 0, t)
        }
        ...
        for (let e in t.nodes) {
            ...
            r && r.forEach((t => {
                n(t = X(t));
                t.node || t.ignore || t.mark || (t.node = e)
            }))
        }
        return e
    }
...
function H(t, e, n) {
    return e != null ? (e ? G : 0) | (e === "full" ? V : 0) : t && t.whitespace == "pre" ? G | V : n & ~_
}
class NodeContext {
    constructor(t, e, n, r, i, s) {
        ...
        this.match = i || (s & _ ? null : t.contentMatch)
    }
    findWrapping(t) {
        if (!this.match) {
            if (!this.type) return [];
            let e = this.type.contentMatch.fillBefore(Fragment.from(t));
            if (!e) {
                let e, n = this.type.contentMatch;
                if (e = n.findWrapping(t.type)) {
                    this.match = n;
                    return e
                }
                return null
    ...
    finish(t) {
        ...
        !t && this.match && (e = e.append(this.match.fillBefore(Fragment.empty, true)));
        return this.type ? this.type.create(this.attrs, e, this.marks) : e
    }
    inlineContext(t) {
...
class ParseContext {
    constructor(t, e, n) {
        ...
        let r, i = e.topNode;
        let s = H(null, e.preserveWhitespace, 0) | (n ? _ : 0);
        r = i ? new NodeContext(i.type, i.attrs, Mark.none, true, e.topMatch || i.type.contentMatch, s) : new NodeContext(n ? null : t.schema.topNodeType, null, Mark.none, true, null, s);
        this.nodes = [r];
    ...
    addTextNode(t, e) {
        let n = t.nodeValue;
        let r = this.top,
            i = r.options & V ? "full" : this.localPreserveWS || (r.options & G) > 0;
        if (i === "full" || r.inlineContext(t) || /[^ \t\r\n\u000c]/.test(n)) {
            if (i) n = i !== "full" ? n.replace(/\r?\n|\r/g, " ") : n.replace(/\r\n?/g, "\n");
            else {
                n = n.replace(/[ \t\r\n\u000c]+/g, " ");
                if (/^[ \t\r\n\u000c]/.test(n) && this.open == this.nodes.length - 1) {
                    let e = r.content[r.content.length - 1];
                    let i = t.previousSibling;
                    (!e || i && i.nodeName == "BR" || e.isText && /[ \t\r\n\u000c]$/.test(e.text)) && (n = n.slice(1))
                }
            }
            n && this.insertNode(this.parser.schema.text(n), e, !/\S/.test(n));
            this.findInText(t)
    ...
    addElement(t, e, n) {
        let r = this.localPreserveWS,
            i = this.top;
        (t.tagName == "PRE" || /pre/.test(t.style && t.style.whiteSpace)) && (this.localPreserveWS = true);
        let s, o = t.nodeName.toLowerCase();
        U.hasOwnProperty(o) && this.parser.normalizeLists && K(t);
        let l = this.options.ruleFromNode && this.options.ruleFromNode(t) || (s = this.parser.matchTag(t, this, n));
        t: if (l ? l.ignore : W.hasOwnProperty(o)) {
            this.findInside(t);
            this.ignoreFallback(t, e)
        } else
        if (!l || l.skip || l.closeParent) {
            l && l.closeParent ? this.open = Math.max(0, this.open - 1) : l && l.skip.nodeType && (t = l.skip);
            let n, r = this.needsBlock;
            if (L.hasOwnProperty(o)) {
                if (i.content.length && i.content[0].isInline && this.open) {
                    this.open--;
                    i = this.top
                }
                n = true;
                i.type || (this.needsBlock = true)
            } else if (!t.firstChild) {
                this.leafFallback(t, e);
                break t
            }
            let s = l && l.skip ? e : this.readStyles(t, e);
            s && this.addAll(t, s);
            n && this.sync(i);
            this.needsBlock = r
        } else {
            let n = this.readStyles(t, e);
            n && this.addElementByRule(t, l, n, l.consuming === false ? s : void 0)
        }
    ...
    ignoreFallback(t, e) {
        t.nodeName != "BR" || this.top.type && this.top.type.inlineContent || this.findPlace(this.parser.schema.text("-"), e, true)
    }
    readStyles(t, e) {
        let n = t.style;
        if (n && n.length)
            for (let t = 0; t < this.parser.matchedStyles.length; t++) {
                ...
                if (i)
                    for (let t;;) {
                        ...
                        e = n.clearMark ? e.filter((t => !n.clearMark(t))) : e.concat(this.parser.schema.marks[n.mark].create(n.attrs));
                        if (n.consuming !== false) break;
                        t = n
                    }
            }
        return e
    ...
    addElementByRule(t, e, n, r) {
        let i, s;
        if (e.node) {
            s = this.parser.schema.nodes[e.node];
            if (s.isLeaf) this.insertNode(s.create(e.attrs), n, t.nodeName == "BR") || this.leafFallback(t, n);
            else {
        ...
        if (s && s.isLeaf) this.findInside(t);
            ...
            this.findInside(t);
            e.getContent(t, this.parser.schema).forEach((t => this.insertNode(t, n, false)))
        } else {
            let r = t;
            typeof e.contentElement == "string" ? r = t.querySelector(e.contentElement) : typeof e.contentElement == "function" ? r = e.contentElement(t) : e.contentElement && (r = e.contentElement);
            this.findAround(t, r, true);
            this.addAll(r, n);
            this.findAround(t, r, false)
        }
        i && this.sync(o) && this.open--
    }
    addAll(t, e, n, r) {
        let i = n || 0;
        for (let s = n ? t.childNodes[n] : t.firstChild, o = r == null ? null : t.childNodes[r]; s != o; s = s.nextSibling, ++i) {
    ...
    findPlace(t, e, n) {
        let r, i;
        for (let e = this.open, s = 0; e >= 0; e--) {
            let o = this.nodes[e];
            let l = o.findWrapping(t);
            if (l && (!r || r.length > l.length + s)) {
                r = l;
                i = o;
                if (!l.length) break
            }
        ...
        if (!r) return null;
        this.sync(i);
        for (let t = 0; t < r.length; t++) e = this.enterInner(r[t], null, e, false);
        return e
    }
    insertNode(t, e, n) {
        ...
        if (r) {
            ...
            let n = Mark.none;
            for (let i of r.concat(t.marks))(e.type ? e.type.allowsMarkType(i.type) : Y(i.type, t.type)) && (n = i.addToSet(n));
            e.content.push(t.mark(n));
            return true
        }
        return false
    }
    enter(t, e, n, r) {
        let i = this.findPlace(t.create(e), n, false);
        i && (i = this.enterInner(t, e, n, true, r));
        return i
    }
    enterInner(t, e, n, r = false, i) {
        this.closeExtra();
        let s = this.top;
        s.match = s.match && s.match.matchType(t);
        let o = H(t, i, s.options);
        s.options & _ && s.content.length == 0 && (o |= _);
        let l = Mark.none;
        n = n.filter((e => {
            if (s.type ? s.type.allowsMarkType(e.type) : Y(e.type, t)) {
                l = e.addToSet(l);
                return false
            }
            return true
        }));
        this.nodes.push(new NodeContext(t, e, l, r, null, o));
        this.open++;
        return n
    }
    closeExtra(t = false) {
    ...
    sync(t) {
        for (let e = this.open; e >= 0; e--) {
            ...
            this.localPreserveWS && (this.nodes[e].options |= G)
        }
        return false
    }
    get currentPos() {
    ...
    findInside(t) {
        if (this.find)
            for (let e = 0; e < this.find.length; e++) this.find[e].pos == null && t.nodeType == 1 && t.contains(this.find[e].node) && (this.find[e].pos = this.currentPos)
    }
    findAround(t, e, n) {
        if (t != e && this.find)
            for (let r = 0; r < this.find.length; r++)
                if (this.find[r].pos == null && t.nodeType == 1 && t.contains(this.find[r].node)) {
    ...
    matchesContext(t) {
        ...
        let i = -(n ? n.depth + 1 : 0) + (r ? 0 : 1);
        let s = (t, o) => {
            for (; t >= 0; t--) {
                let l = e[t];
                if (l == "") {
                    if (t == e.length - 1 || t == 0) continue;
                    for (; o >= i; o--)
                        if (s(t - 1, o)) return true;
                    return false
                } {
                    let t = o > 0 || o == 0 && r ? this.nodes[o].type : n && o >= i ? n.node(o - i).type : null;
                    if (!t || t.name != l && !t.isInGroup(l)) return false;
                    o--
                }
            }
            return true
...
function K(t) {
    for (let e = t.firstChild, n = null; e; e = e.nextSibling) {
        let t = e.nodeType == 1 ? e.nodeName.toLowerCase() : null;
        if (t && U.hasOwnProperty(t) && n) {
            n.appendChild(e);
            e = n
        } else t == "li" ? n = e : t && (n = null)
    }
}
        ...
        let s = [],
            o = t => {
                ...
                for (let n = 0; n < t.edgeCount; n++) {
                    ...
                    if (r == e) return true;
                    if (s.indexOf(i) < 0 && o(i)) return true
                }
            };
...
class DOMSerializer {
    ...
    serializeFragment(t, e = {}, n) {
        ...
        t.forEach((t => {
            if (i.length || t.marks.length) {
                let n = 0,
                    s = 0;
                while (n < i.length && s < t.marks.length) {
                    let e = t.marks[s];
                    if (this.marks[e.type.name]) {
                        if (!e.eq(i[n][0]) || e.type.spec.spanning === false) break;
                        n++;
                        s++
                    } else s++
                }
                while (n < i.length) r = i.pop()[1];
                while (s < t.marks.length) {
                    let n = t.marks[s++];
                    let o = this.serializeMark(n, t.isInline, e);
                    if (o) {
                        i.push([n, r]);
                        r.appendChild(o.dom);
    ...
    serializeNodeInner(t, e) {
        let {
            dom: n,
            contentDOM: r
        } = it(tt(e), this.nodes[t.type.name](t), null, t.attrs);
        if (r) {
    ...
    serializeNode(t, e = {}) {
        ...
        for (let r = t.marks.length - 1; r >= 0; r--) {
            ...
            if (i) {
                (i.contentDOM || i.dom).appendChild(n);
                n = i.dom
            }
        }
        return n
    ...
    serializeMark(t, e, n = {}) {
        ...
        return r && it(tt(n), r(t, e), null, t.attrs)
    }
    static renderSpec(t, e, n = null, r) {
        return it(t, e, n, r)
    }
    static fromSchema(t) {
        return t.cached.domSerializer || (t.cached.domSerializer = new DOMSerializer(this.nodesFromSchema(t), this.marksFromSchema(t)))
    }
    static nodesFromSchema(t) {
        let e = Z(t.nodes);
        e.text || (e.text = t => t.text);
        return e
    }
...
function it(t, e, n, r) {
    ...
    if (r && (i = nt(r)) && i.indexOf(e) > -1) throw new RangeError("Using an array from an attribute object as a DOM spec. This may be an attempted cross site scripting attack.");
    let o = s.indexOf(" ");
    if (o > 0) {
        n = s.slice(0, o);
        s = s.slice(o + 1)
    ...
    let l;
    let h = n ? t.createElementNS(n, s) : t.createElement(s);
    let a = e[1],
        c = 1;
    if (a && typeof a == "object" && a.nodeType == null && !Array.isArray(a)) {
        ...
        for (let t in a)
            if (a[t] != null) {
                let e = t.indexOf(" ");
                e > 0 ? h.setAttributeNS(t.slice(0, e), t.slice(e + 1), a[t]) : h.setAttribute(t, a[t])
            }
    }
    for (let i = c; i < e.length; i++) {
        let s = e[i];
        if (s === 0) {
            if (i < e.length - 1 || i > c) throw new RangeError("Content hole must be the only child of its parent node");
            return {
                dom: h,
                contentDOM: h
            }
